...
class C_sockethandler :public fd_atom, public hook_callback::logic_handler, public ssl_hook::ssl_handler
{
    ...
    void on_write_end(int fd, ConnectionInfo *conn, char *buf, size_t len, UINT64 start_time, ssize_t ret);
...
public:
    ...
    void hand_SSL_write(void *ssl,const void *buf,int num, int ret, UINT64 start_time, UINT64 end_time);
    ...
    virtual void on_ssl_read(void *ssl,void *buf,int num, int ret, UINT64 start_time, UINT64 end_time);
    virtual void on_ssl_write(void *ssl,const void *buf,int num, int ret, UINT64 start_time, UINT64 end_time);
