...
struct RequestInfo : public allocator_l0::contain_base {
    ...
    RequestInfo(allocator_l0::allocator *Allocate) : start_time(0), request(Allocate), recved(0),
                                                     response_time(0), upload(0),
                                                     req_end_time(0) { }
...
//     } addr;
    ...
    ConnectionInfo(allocator_l0::allocator *Allocator = 0)
            : start_time(0), connect_start(0), connect_end(0), _err_num(0), curr_time(0),
              _connected(false), _fd(-1), _ssl(0), _use_ssl(false), _ref_count(1) {
        set_allocator(Allocator);
        _response._requests = &_requests;
    }
